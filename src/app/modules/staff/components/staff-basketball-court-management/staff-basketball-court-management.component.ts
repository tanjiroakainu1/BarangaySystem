...
export class StaffBasketballCourtManagementComponent implements OnInit {
  ...
  updateReservationStatusFromSelect(reservationId: string, event: Event) {
  ...
  updateReservationStatus(reservationId: string, status: BasketballCourtReservation['status']) {
    const notes = prompt('Add notes (optional):');
    this.basketballCourtService.updateReservationStatus(
      reservationId,
      status,
      notes || undefined,
      'Staff User'
      ...
      error: (err) => alert(err?.message || 'Failed to update reservation status'),
    });
  }
