...
-- >>> FILE: 22_list_staff_data_rpcs.sql >>>
...
BEGIN
  ...
  END IF;
  RETURN QUERY SELECT * FROM public.appointment_requests ORDER BY created_at DESC;
END;
$$;
...
BEGIN
  ...
  END IF;
  RETURN QUERY SELECT * FROM public.certificates ORDER BY created_at DESC;
END;
$$;
...
BEGIN
  ...
  END IF;
  RETURN QUERY SELECT * FROM public.basketball_court_reservations ORDER BY created_at DESC;
END;
$$;
